# -*- coding: utf-8 -*-
"""
Created on Fri Sep 21 15:57:47 2018
...
"""
classes = [item for item in os.listdir(root) if os.path.isdir(os.path.join(root, item)) and item!="unks"]
...
maxitr = 3         #maximum number of epochs
...
"""
X = tf.placeholder(tf.double, (None, 2, 128, 128, 3))
Y = tf.placeholder(tf.double, (None,2))
  ...
  for pathi in range(len(classes[:maxclasses])):
    ...
    for i in range(2):
      for j in range(i+1,2):
        img1 = imread(imagepaths[i],1).reshape((1,imsize,imsize,ch))
        img2 = imread(imagepaths[j],1).reshape((1,imsize,imsize,ch))
        if(np.array_equal(img1,img2)):
          continue     
        sess.run(algo, feed_dict = {X = np.stack((img1,img2)), Y : np.ones((2,1))})
        
    for pathj in range(pathi+1, min(pathi+1+maxnextclasses,len(classes[:maxclasses]))):
      imagepathsj = imPaths(pathj)
      for i in range(2):
        for j in range(2):
          img1 = imread(imagepaths[i],1).reshape((1,imsize,imsize,ch))
          img2 = imread(imagepathsj[j],1).reshape((1,imsize,imsize,ch))
          if(np.array_equal(img1,img2)):
            continue
          labelar = np.stack((int(classes[pathi]),int(classes[pathj]))).reshape((1,2))
          sess.run(algo, feed_dict = {X = np.stack((img1,img2)), Y : )})
      imagepathsx = imPaths(np.random.randint(0,maxclasses+1))
  print('epoch '+str(itr+1)+' done')
  
"""
Save model to above root directory, use only when required
"""
saver.save(sess, roots)

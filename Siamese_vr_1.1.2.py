...
maxclasses = 10  # number of classes to use of the classes given
...
fcon0 = flatten(pool4)
fcon1 = tf.layers.dense(fcon0,fconshape1, name = 'fcon1')

fcon0x = flatten(pool4x)
fcon1x = tf.layers.dense(fcon0x,fconshape1, name = 'fcon1', reuse = True)
...
for itr in range(maxitr):
  ...
  for pathi in range(len(classes[:maxclasses])):
    ...
    sameclasses = np.random.randint(0,num_classes,(10))
    crossclasses = np.random.randint(0,num_classes,(10))
    crossclasses = crossclasses[crossclasses!=pathi]
    for k in range(len(crossclasses)):
      imagepathsj = imPaths(crossclasses[k])
      imagepathsx = imPaths(sameclasses[k])
      for i in range(2):
        for j in range(2):
          img1 = imRead(imagepaths[i], image_size, num_channels)
          ...
          img1 = imRead(imagepathsx[i], image_size, num_channels)
          img2 = imRead(imagepathsx[j], image_size, num_channels)
          if(np.array_equal(img1,img2)):
            continue     
          sess.run(algo, feed_dict = {x : img1, x1 : img2, y : np.ones((1,1)), y1 : np.ones((1,1))})

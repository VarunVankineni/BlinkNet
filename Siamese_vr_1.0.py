...
for itr in range(maxitr):
  ...
  for pathi in range(len(classes[:maxclasses])):
    ...
    for pathj in range(pathi+1, len(classes[:maxclasses])):
...
for i in range(vecs.shape[0]):
  for j in range(vecst.shape[0]):
    Eucdist[j,i] = np.linalg.norm(vecs[i]-vecst[j])
for i in range(vecs.shape[0]):
  for j in range(vecs1.shape[0]):
    Eucdist[j+vecst.shape[0],i] = np.linalg.norm(vecs[i]-vecs1[j])
